...
//    private JdbcTemplate jdbcTemplate;
    ...
    public boolean auth(String key, String loginData) {
    	boolean authResult= false;
        if( loginData != null ){
        	try {
	            byte[] srcMsg = SecurityUtil.desDecrypt(key, StringUtil.hexStringToBytes(loginData));
	            
	            String authString = new String(srcMsg);
         ...
         ResultSet rs = null;
     	 try{
     	     if( this.sql  == null || sql.length() == 0){
         		 this.sql = "SELECT " + fieldPassword + " from " + this.tableUsers + " Where " + this.fieldUser  + " = ? ";
         		 if(isAvailable != null && isAvailable.length() > 0){
         		    sql = sql + " AND " +isAvailable + " in ( " + isAvailableValue+ ")";
         		 }
     	     }
     		String dbPassword ="";
     		logger.info(sql);
...
//        org.jasig.cas.authentication.handler.Md5PasswordEncoder e = new org.jasig.cas.authentication.handler.Md5PasswordEncoder();
